...
func FavoriteAction(userId uint, videoId uint, actionType uint) (err error) {
	...
	if actionType == 1 {
		...
		result := dao.Db.Table("favorites").Where("user_id = ? AND video_id = ?", userId, videoId).First(&favoriteExist)
		if result.Error != nil { // 不存在
			if err := dao.Db.Table("favorites").Create(&favoriteAction).Error; err != nil { // 创建记录
				return err
			}
			dao.Db.Table("videos").Where("id = ?", videoId).Update("favorite_count", gorm.Expr("favorite_count + 1"))
			...
			if favoriteExist.State == 0 { // state为0-video的favorite_count加1
				dao.Db.Table("videos").Where("id = ?", videoId).Update("favorite_count", gorm.Expr("favorite_count + 1"))
				dao.Db.Table("favorites").Where("video_id = ?", videoId).Update("state", 1)
				// userId的favorite_count增加
				if err := AddFavoriteCount(userId); err != nil {
					return err
				}
		...
		if err := dao.Db.Table("favorites").Where("user_id = ? AND video_id = ?", userId, videoId).First(&favoriteCancel).Error; err != nil { // 找不到这条记录，取消点赞失败，创建记录
			dao.Db.Table("favorites").Create(&favoriteActionCancel)
			// userId的favorite_count增加
		...
		if favoriteCancel.State == 1 { // state为1-video的favorite_count减1
			dao.Db.Table("videos").Where("id = ?", videoId).Update("favorite_count", gorm.Expr("favorite_count - 1"))
			// 更新State
			dao.Db.Table("favorites").Where("video_id = ?", videoId).Update("state", 0)
			if err := ReduceFavoriteCount(userId); err != nil {
				return err
			}
			// videoId对应的userId的total_favorite增加
...
func FavoriteList(userId uint) ([]model.Video, error) {
	...
	if err := dao.Db.Table("favorites").Where("user_id=? AND state=?", userId, 1).Find(&favoriteList).Error; err != nil { // 找不到记录
		return videoList, nil
	}
	for _, m := range favoriteList {

		var video = model.Video{}
		if err := dao.Db.Table("videos").Where("id=?", m.VideoId).Find(&video).Error; err != nil {
			return nil, err
		}
