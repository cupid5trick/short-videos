...
func CreateRegisterUser(userName string, passWord string) (model.User, error) {
	...
	if IsUserExistByName(userName) {
		...
		// 用户不存在，新建用户
		if err := dao.Db.Model(&model.User{}).Create(&newUser).Error; err != nil {
			// 错误处理
			// fmt.Println(err)
			panic(err)
			return newUser, err
...
func IsUserExistByName(userName string) bool {
	...
	if err := dao.Db.Model(&model.User{}).Where("name=?", userName).First(&userExist).Error; errors.Is(err, gorm.ErrRecordNotFound) {
		// 关注不存在
		return false
	}
	// 关注存在
...
func IsUserExist(userName string, password string, login *model.User) error {
	...
	dao.Db.Where("name=?", userName).First(login)
...
func GetUser(userId uint) (model.User, error) {
	...
	if err := dao.Db.Model(&model.User{}).Where("id = ?", userId).Find(&user).Error; err != nil {
		return user, err
	}
	return user, nil
}
